...
class Adapter(persist.Adapter):
    ...
    def load_policy(self, model):
        ...
        # lines = RoleRule.objects.all()

        # for line in lines:
        #     persist.load_policy_line(str(line), model)

    def _create_policy_line(self, ptype, rule):
        line = RoleBind(ptype=ptype)
        if len(rule) > 0:
            line.v0 = rule[0]
        if len(rule) > 1:
            line.v1 = rule[1]
    ...
    def save_policy(self, model):
        ...
        RoleBind.objects.bulk_create(lines)
        return True
    ...
    def remove_policy(self, sec, ptype, rule):
        ...
        rows_deleted, _ = RoleBind.objects.filter(**query_params).delete()
